...
#' @param level level used in predict() fit an nlme object
#' 
#' @import foreach
#' @import parallel
#' @import doParallel
...
analyzeNLME <- function(data, 
    ...
    if (is.null(level)) { stop("Level for prediction using nlme must be specified!") }
    ...
    for (lv in lvs) {
        ...
        res <- foreach(varIndex=columns) %dopar% {
            ...
            tryCatch({
                ## use ML for LRT 
                fit0 <- lme(frm0, data=df, method="ML", random=rand)
                fit <- lme(frm, data=df, method="ML", random=rand)
                a <- anova(fit0, fit)

                ## use REML as reference
                fit <- lme(frm, data=df, method="REML", random=rand)
                R2CV <- NULL
                ...
                pTot <- foreach (lv2=cvLv) %dopar% { 
                    ...
                    #######
                    tryCatch({
                        fit1 <- lme(frm, data=dfTrain, random=rand)
                        prdTmp <- predict(fit1, newdata=dfPred, allow.new.levels=T, level=level) 
                    }, error=function(e) {
                ...
                out <- data.frame(summary(fit)$tTable[-1,,drop=F], 
                                  VAR=colnames(data)[varIndex], 
                                  aP=a[2,9], R2cv=R2CV,
                                  LV=lv)
